...
var allNecessaryKeysAvailable = function() {
  if (Meteor.settings['private'].rollbarApiSecret) {
    return true;
  }
...
Meteor.startup(function() {
  if (allNecessaryKeysAvailable()) {
    rollbarServerAccessToken = process.env.ROLLBAR_SERVER_ACCESS_TOKEN || Meteor.settings['private'].rollbarApiSecret;
    rollbarClientAccessToken = process.env.ROLLBAR_CLIENT_ACCESS_TOKEN || Meteor.settings['public'].rollbarApiKey;
